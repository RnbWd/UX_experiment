...
var Center = React.createClass({displayName: 'Center',
  ...
  getDefaultProps: function() {
    return {circle: 'passive', vertical: 'passive', mandala: 'active'}
  },
  ...
  handleInput: function(event) {
    if (event.target.value == "Circle") {
      this.setProps({circle: 'active'})
    } else if (event.target.value != "Circle") {
      this.set.Props({circle: 'passive'})
    }
     if (event.target.value == "Mandala") {
      this.setProps({mandala: 'active'})
    } else {
      this.set.Props({mandala: 'passive'})
    }
     if (event.target.value == "Vertical") {
      this.setProps({vertical: 'active'})
    } else {
      this.set.Props({vetical: 'passive'})
    }
  ...
  render: function() {
    ...
    return (
      React.DOM.div( {className:"container", onClick:this.handleClick, onWheel:this.handleScroll} , 
        ...
        React.DOM.span( {className:this.props.circle}, "Circle"),
        React.DOM.span( {className:this.props.vertical}, "Vertical"),
        React.DOM.span( {className:this.props.mandala}, "Mandala"),
        Progress( {side:value} ),
